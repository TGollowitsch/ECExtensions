...
namespace ECExtensions
{
	public class ECFunctions
	{
		...
		private (BigInteger x, BigInteger y) PointToTuple(in ECPoint p) =>
			(new BigInteger(p.X, true, true), new BigInteger(p.Y, true, true));

		private ECPoint TupleToPoint(in (BigInteger x, BigInteger y) p) =>
			new ECPoint()
			{
		...
		private bool IsOnCurve(in (BigInteger x, BigInteger y) p) =>
		...
		public bool IsOnCurve(in ECPoint point)
		{
		...
		private void Negate(out (BigInteger x, BigInteger y) p)
		{
			p.y = (-p.y).Mod(prime);
		}
		...
		public void Negate(ref ECPoint point)
		{
			if (!IsOnCurve(point))
				throw new Exception();
			var y = new BigInteger(point.Y, true, true);
			point.Y = (-y).Mod(prime).ToByteArray(true, true);
		}

		private (BigInteger x, BigInteger y) PointAdd(in (BigInteger x, BigInteger y) p, in (BigInteger x, BigInteger y) q) // P + Q = R
		{
		...
		public ECPoint Add(in ECPoint point, in ECPoint addend) // P + Q = R
		{
			var p = PointToTuple(point);
		...
		private (BigInteger x, BigInteger y) ScalarMultiply(in (BigInteger x, BigInteger y) p, BigInteger n)
		{
			if (!IsOnCurve(p))
				throw new Exception();
		...
		public ECPoint Multiply(in ECPoint point, in BigInteger n)
		{
			var p = PointToTuple(point);
		...
		public byte[] Serialize(in ECPoint point, bool compressedForm)
		{
			if (compressedForm)
			{
